...
class SparseVGAE(VGAE):
    ...
    def loss(self, latent, pt,
        ...
        ortho_loss = self.get_ortho_loss(latent.qz)
        ...
        reg_loss = self.get_l1_regularization()
    ...
    def get_l1_regularization(self):
        ...
        for param in self.parameters():
            loss += param.view(-1).abs().sum()
        return self.l1_weight * loss
...
class GCNEncoder(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight):
        # q(pi | x, A) & q(b | \pi)
        qc1 = F.softplus(self.x_to_c1(x, edge_index, edge_weight)) + EPS
        qc0 = F.softplus(self.x_to_c0(x, edge_index, edge_weight)) + EPS
        ...
        # q(z | b, x, A)
        qz_loc = F.sigmoid(self.x_to_zloc(x, edge_index, edge_weight))
